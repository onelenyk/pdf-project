...
@Command(
    name = "pdfproject", mixinStandardHelpOptions = true, version = ["pdfproject 1.0"],
    description = ["A command-line tool to convert any project to a PDF document."]
)
class App : Callable<Int> {

    @Option(names = ["-p", "--projectRoot"], description = ["Path to the project root"], required = true)
    lateinit var projectRoot: String

    @Option(names = ["-r", "--rules"], description = ["Custom rules for filtering files"], split = ",")
    var customRules: List<String> = defaultRules

    override fun call(): Int {
        val projectRootPath = Paths.get(projectRoot).toAbsolutePath()
        val converter = ProjectToPdfExporter(
            projectRootPath,
